...
class Form
{
    ...
    protected function getRelationInputs($inputs = [])
    {
        ...
        foreach ($inputs as $original_column => $value) {
            $column = camelize($original_column);

            if (method_exists($this->model, $column)) {
                $relation = call_user_func([$this->model, $column]);

                if ($relation instanceof Relation) {
                    $relations[$original_column] = $value;
                }
            }
        }
    ...
    /**
     * Call relationMaking callback.
     *
     * @return mixed
     */
    protected function callRelationMaking($attributes=[])
    {
        if ($this->relationMaking instanceof Closure) {
            return call_user_func($this->relationMaking, $attributes);
        }
    }

    /**
     * Call customizing callback.
     *
     * @return mixed
     */
    protected function callCustomizing($attributes=[])
    {
        if ($this->customizing instanceof Closure) {
            return call_user_func($this->customizing, $attributes);
        }
    }
    ...
    {
        $rebuild_requset_all = collect();
        $requset_all = collect(Input::all());

        $requset_all->each(function ($item, $original_column) use (&$rebuild_requset_all) {
            if (in_array($original_column, ['_token', '_method', '_previous_'])) {
                $rebuild_requset_all->put($original_column, $item);
                return ;
            }

            $column = explode('_', $original_column);

            if (isset($column[1]) && $column[1] == 'batch') {
                $column[0] = uncamelize($column[0]);
                $rebuild_requset_all->put($column[0], $rebuild_requset_all->get($column[0]) ? : collect());

                collect($item)->each(function ($_item, $_column) use (&$rebuild_requset_all, $column) {
                    $rebuild_requset_all->get($column[0])->push($_item, $_column);
                });
            } else {
                $rebuild_requset_all->put($original_column, $item);
            }
        ...
        return $rebuild_requset_all->recursiveAll();
    }
    ...
    protected function updateRelation($relationsData)
    {
        foreach ($relationsData as $name => $values) {
            ...
            if (!method_exists($this->model, camelize($name))) {
                continue;
            }

            $relation = $this->model->{camelize($name)}();
            ...
            switch (get_class($relation)) {
                ...
                case \Illuminate\Database\Eloquent\Relations\HasMany::class:
                    ...
                    foreach ($prepared[$name] as $related) {
                        ...
                        if ($relationMaking = $this->callRelationMaking($related)) {
                            $related = array_merge($relationMaking, $related);
                        }
    ...
    protected function prepareUpdate(array $updates, $hasDot = false)
    {
        ...
        $prevent_repeat = collect();

        foreach ($this->builder->fields() as $field) {
            $columns = $field->column();

            if ($prevent_repeat->contains($columns)) { // TODO: check
                continue;
            }
            $prevent_repeat->push($columns);
    ...
    /**
     * Set relationMaking callback.
     *
     * @param Closure $callback
     *
     * @return void
     */
    public function relationMaking(Closure $callback)
    {
        $this->relationMaking = $callback;
    }

    /**
     * Set customizing callback.
     *
     * @param Closure $callback
     *
     * @return void
     */
    public function customizing(Closure $callback)
    {
        $this->customizing = $callback;
    }
    ...
    public function getRelations()
    {
        ...
        foreach (array_flatten($columns) as $key => $column) {
            ...
            if (str_contains($column, '.')) {
                ...
                $field = $this->builder->fields()->get($key);
                if ($field instanceof Field\HasMany && ($custom_with = $field->getCustomWith())) {
                    list($column, $value) = each($custom_with);

                    $relations[$column] = $value;
                } else {
                    $relations[] = $column;
                }
            } elseif ($field instanceof Field\HasMany && ($relation_name = $field->getRelationName())) {
    ...
    public function __call($method, $arguments)
    {
        if ($className = static::findFieldClass($method)) {
            ...
            // If there is a customizing closure set, the call it.
            if ($this->customizing instanceof Closure) {
                array_push($arguments, $this->customizing);
