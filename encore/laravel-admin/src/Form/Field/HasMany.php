...
class HasMany extends Field
{
    ...
    public function __construct($relationName, $arguments = [])
    {
        ...
        if (count($arguments) == 3) {
            list($this->label, $this->builder) = array_slice($arguments, 0,2);
            $arguments[2]($this);
        }
    }

    /**
     * Set custom_with attribute
     * @param $custom_with
     */
    public function setCustomWith($custom_with)
    {
        $this->custom_with = $custom_with;
    }

    /**
     * Set relationName attribute
     */
    ...
    /**
     * Get custom_with attribute
     * @param $custom_with
     */
    public function getCustomWith()
    {
        return $this->custom_with;
    }
    ...
    {
        $this->groupBy = is_string($value) ? func_get_args() : $value;
    }
    ...
    protected function buildRelatedForms()
    {
        ...
        if ($values = old($this->column)) {
            ...
            // Group By data
            if (is_array($this->groupBy) && $this->value) {
                list($group_by, $item) = each($this->groupBy);
                $group_by_data = collect($this->value)
                    ->groupBy($group_by);

                if ($group_by_data->has($item)) {
                    $value = $group_by_data
                        ->get($item)
                        ->all();
                } else {
